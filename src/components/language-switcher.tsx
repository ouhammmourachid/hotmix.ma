...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ className = '' }) => {
  ...
  return (
    ...
    >
      ...
      {/* Dropdown content */}
      {isOpen && (
        <div className={`absolute top-full ${language === 'ar' ? '-right-16' : 'right-0'} mt-1 bg-primary shadow-lg rounded-sm overflow-hidden z-50 min-w-32 py-1`}>
          {languages.map((lang) => (
            <button
              key={lang.code}
              onClick={() => handleLanguageChange(lang.code)}
              className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:text-greny ${language === lang.code ? 'text-greny font-medium' : ''
                }`}
            >
